...
// Algorithm-
// It is almost the same as using priority queue, since set stores in ascending order
// Also, set can erase the pair with same node but more distance whereas priority queue does not
// 1. Create a set to store pair of distance of node and node
// 2. create a distance array and store 0 for source node and rest store infinity
// 3. take out the node from set and go to its adjacent node and store min(current distance, distance already stored)
// 4. store the pair of the distance of adjacent node and the adjacent node in the set
